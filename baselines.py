...
from utils import cast_ner_labels_to_int, words_to_offsets
...
def create_embeddings(tokens, embed_model, window_size):
...
def split_examples(examples, embed_model, window_size):
    ...
    for tokens, labels in zip(examples["tokens"], examples["ner_tags"]):
        embeddings = create_embeddings(tokens, embed_model, window_size)
...
class MLBaseline:
    ...
    def predict(self, sentences: List[str]):
        ...
        for n_sent, sentence in enumerate(sentences):
            sentence = sentence.split()
            embeddings = create_embeddings(sentence, self.embed_model, self.window_size)
